...
- **Контроль AI.** Для модели ограничены контекстное окно и резерв ответа; backend проверяет JSON и применяет fallback без частичной записи состояния. На Уровнях 3–4 и в Свободной игре модель используется только Evaluator-ом, а тематическую реплику выбирает детерминированный Generator.
