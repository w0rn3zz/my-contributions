...
- generator модель не вызывает;
- evaluator output: до 120 токенов;
...
- транспортная ошибка модели не повторяется внутри того же HTTP-запроса, не меняет состояние и позволяет Пользователю повторить действие; повторно невалидный evaluator использует детерминированную консервативную fallback-оценку `score=1..3`, а Generator выбирает безопасную fallback-реплику Сценария без model inference.
