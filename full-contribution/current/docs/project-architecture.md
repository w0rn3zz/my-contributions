...
- транспортная ошибка модели не меняет состояние и запрос можно повторить; повторно невалидный evaluator использует детерминированную консервативную fallback-оценку `score=1..3` по тексту Ответа пользователя, а Сценарий хранит безопасную fallback-реплику.
